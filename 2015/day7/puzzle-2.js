...
const contents = file.getLines(path.join(__dirname, "input2.txt"), "\n", "");
...
const runInstructions = (database, instructions) => {
  while (instructions.length > 0) {
    let pending = [];
    for (let instruction of instructions) {
      let wasExecuted = run(instruction, database);
...
let db = {};
runInstructions(db, instructions);
