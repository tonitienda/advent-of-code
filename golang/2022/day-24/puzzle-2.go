...
func bfsPoi(start, goal Cell, cycle, minute int, lookDown bool) int {
	...
	for {
		...
		for _, node := range pendingNodes {
			...
			for _, n := range neighbours {
				...
				if lookDown && n.row == goal.row-1 && n.col == goal.col {
...
func main() {
	...
	minutes := bfsPoi(entrance, exit, cycle, 0, true)
	minutes = bfsPoi(exit, entrance, cycle, minutes, false)
	minutes = bfsPoi(entrance, exit, cycle, minutes, true)
