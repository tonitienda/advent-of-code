...
func getCorrespondence(indirections [][3]int, s int) int {
...
func main() {
	...
	for _, seed := range seeds {
		...
		location := getCorrespondence(humidityToLocation,
			getCorrespondence(temperatureToHumidity,
				getCorrespondence(lightToTemperature,
					getCorrespondence(waterToLight,
						getCorrespondence(fertilizerToWater,
							getCorrespondence(soilToFertilizer,
								getCorrespondence(seedsToSoil, seed)))))))
