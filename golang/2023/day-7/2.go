...
func cardNumbersToMap(cards [5]byte) map[byte]byte {
	...
	highestCard := byte(0)
	highestNumCards := byte(0)
	jCode := cardToPoint[byte('J')]

	for _, card := range cards {
		m[card] = m[card] + 1

		if card != jCode && highestNumCards < m[card] {
			highestNumCards = m[card]
			highestCard = card
		}
	}
	...
	fmt.Println("map before:", m, "highestCard", highestCard)

	m[highestCard] += numberOfJs
	delete(m, jCode)

	fmt.Println("Cards:", cards, "Js:", numberOfJs, "map:", m)
